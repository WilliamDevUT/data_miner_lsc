...
def transcribe_single_chunk(client, chunk, chunk_num, total_chunks) -> tuple[dict, float]:
    """
    ...
    while True:
        # 1. Crear archivo temporal con delete=False para que no se borre al cerrar
        # Esto permite cerrarlo para liberar el "candado" de Windows
        temp_file = tempfile.NamedTemporaryFile(suffix='.flac', delete=False)
        temp_path = temp_file.name
        temp_file.close()  # <--- CRUCIAL: Cerramos el archivo aquí

        try:
            # 2. Exportamos el audio (ahora pydub puede abrirlo porque está cerrado)
            chunk.export(temp_path, format='flac')
            
            start_time = time.time()
            
            # 3. Abrimos el archivo en modo lectura binaria para enviarlo a la API
            with open(temp_path, "rb") as audio_file:
                try:
                    result = client.audio.transcriptions.create(
                        # Pasamos el objeto de archivo abierto 'audio_file'
            ...
            # 4. LIMPIEZA: Borramos el archivo manualmente pase lo que pase
            # Esto se ejecuta tanto si hubo éxito como si hubo error
...
def save_results(result: dict, audio_path: Path, final_output_path) -> Path:
    """
    ...
    Args:
        ...
        final_output_path: Directory where to save results
    ...
    try:
        ...
        # Nombre fijo siempre: "audio"
        base_path = output_dir / "audio"
        ...
        raise
def transcribe_audio_in_chunks(audio_path: Path, final_output_path: Path, chunk_length: int = 600, overlap: int = 10 ) -> dict:
    """
    ...
    try:
        ...
        save_results(final_result, audio_path, final_output_path)
...
if __name__ == "__main__":
    load_dotenv()
    #transcribe_audio_in_chunks(Path(r"C:\Users\willd\OneDrive\Documentos\my\congreso\new\data_miner\vid_data\video_final2443\audio.wav"))